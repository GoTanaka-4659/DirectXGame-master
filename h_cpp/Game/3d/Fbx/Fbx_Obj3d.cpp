...
//静的メンバ変数の実態
ID3D12Device *Fbx_Obj3d::device = nullptr;
//Camera *Fbx_Obj3d::camera = nullptr;
// ルートシグネチャ
ComPtr<ID3D12RootSignature> Fbx_Obj3d::rootsignature;
// パイプラインステートオブジェクト
ComPtr<ID3D12PipelineState> Fbx_Obj3d::pipelinestate;

void Fbx_Obj3d::Initilize()
{
	HRESULT result;
...
void Fbx_Obj3d::Draw(ID3D12GraphicsCommandList *cmdList)
{
	// モデルの割り当てがなければ描画しない
	if (model == nullptr) {
		return;
...
void Fbx_Obj3d::CreateGraphicsPipeline()
{
