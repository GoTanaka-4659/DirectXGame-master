...
void FbxLoader::ParceMaterial(FBXModel *model, FbxNode *fbxnode)
{
...
FBXModel *FbxLoader::LoadModelFromFile(const string &modelName)
{
    ...
    //あらかじめ必用数分のメモリを確保することで、アドレスがずれるのを予防
...
void FbxLoader::ParseSkin(FBXModel *model, FbxMesh *fbxmesh)
{
    ...
    for (int i = 0; i < clusterCount; i++) {
        ...
        //FBXから初期姿勢行列を取得する
        FbxAMatrix fbxMat;
        fbxCluster->GetTransformAssociateModelMatrix(fbxMat);
        //XMMatrix型に変換する
